...
export const validate =  
    (validators: ValidatorArray, asyncValidators: AsyncValidatorArray, userDefinedMessage:ValidationResult) => {
  return (control: AbstractControl) => {
    customMessages = userDefinedMessage;
...
export const message = (validator: ValidationResult, key: string): string => {  
  ...
  if(customMessages[key]){
    return <string> customMessages[key];
  }
  switch (key) {
    case 'required':
...
export let customMessages:ValidationResult;